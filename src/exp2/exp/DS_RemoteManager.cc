...
size_t DS_RemoteManager::writeRead(boost::asio::ip::tcp::socket *s, char request[], char reply[])
{
  ...
  size_t reply_length = s->read_some(boost::asio::buffer(reply, request_length*10), error);
...
StatusCode DS_RemoteManager::handleProtoReq(ProtocolParseObj &ppo, char repbuf[])
{
  ...
  if (ppo.code == 50) {
    ...
    cout << "thoda contents iila: " << contents << std::endl;
    makeProtoHelper(DS_SUCC_REM_READ_CODE, (void *)(&ppo.pageNum), ppo.fileName, repbuf);
    cout << "abhi tak ye banaya : " << repbuf << std::endl;
    strcat(repbuf, "|");
    strcat(repbuf, contents);
    cout << "kya hua : " << repbuf << std::endl;
    cout << "size hai : " << strlen(repbuf) << std::endl;
    // vector<string> strs;
    // boost::algorithm::split(strs,repbuf, boost::is_any_of("|"));
